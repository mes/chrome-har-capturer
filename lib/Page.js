    ...
    for (var requestId in this.entries) {
        ...
        if (!entry.responseEvent || !entry.responseFinished) {
            ...
            har.entries.push({
                ...
                'time': entry.responseFinished ? Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000) : null,
                'request': {
                ...
                'timings': {
                    ...
                    'connect': -1,
                    'send': -1,
                    'wait': entry.responseFinished ? Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000) : null,
                    'receive': -1,
                    'ssl': -1
                }
            });
