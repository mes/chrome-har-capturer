...
Page.prototype.getHAR = function () {
    ...
    for (var requestId in this.entries) {
        ...
        if (!entry.responseFinished) continue;
        ...
        if (!entry.responseEvent) {
            ...
            har.entries.push({
                ...
                'time': Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000),
                'request': {
                ...
                'response': {
                    'status': 500, // harstorage interprets failed requests as >400
                ...
                'timings': {
                    ...
                    'connect': -1,
                    'send': -1,
                    'wait': Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000),
                    'receive': -1,
                    'ssl': -1
                }
            });
