...
Page.prototype.processMessage = function (message) {
    ...
    switch (message.method) {
        ...
        case 'Network.loadingFailed':
            if (id in this.entries) {
                this.entries[id].responseFinished = message.params.timestamp;
                this.entries[id].errorText = message.params.errorText;
                break; // just log dump
            }
            return;
        default:
...
Page.prototype.getHAR = function () {
    ...
    for (var requestId in this.entries) {
        ...
        if (!entry.responseEvent || !entry.responseFinished) {
            // analyze headers
            ...
            har.entries.push({
                ...
                'startedDateTime': new Date(entry.requestEvent.timestamp * 1000).toISOString(),
                'time': entry.responseFinished ? Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000) : -1,
                'request': {
                ...
                'response': {
                    'status': 418, // I'm a teapot - harstorage interprets failed requests as >400
                    'statusText': entry.errorText ? entry.errorText : "",
                    'httpVersion': 'HTTP/1.1', // TODO
                    'cookies': [], // TODO
                    'headers': [],
                    'redirectURL': '', // TODO
                ...
                'timings': {
                    ...
                    'connect': -1,
                    'send': -1,
                    'wait': entry.responseFinished ? Math.round((entry.responseFinished - entry.requestEvent.timestamp) * 1000) : -1,
                    'receive': -1,
                    'ssl': -1
                }
            });
